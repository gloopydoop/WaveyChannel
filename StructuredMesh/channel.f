...
!	   argx = argx/lam
		...
		added = (2*h+wave_amp-ym1(i,1,1,1))/(2*h+wave_amp)
  		ym1(i,1,1,1) = ym1(i,1,1,1) + added*A1
	enddo


	param(54) = -1   ! x-direction
	param(55) = 1 ! HarryDancesLikeAGod
...
!======================================================================
	...
	u_tau  = sqrt(tw)
	Re_tau = Re_d/2*u_tau
	write(6,*) 'dissipation numbers', F, tw, total_dis
      return
      end
...
!	tw = dragx(1)/(wall_area-lam) + dragx(2)/lam
	...
	u_tau  = sqrt(tw)
	Re_tau = Re_d/2*u_tau
	write(6,*) 'Friction numbers', drag_total, tw
