...
!-----------------------------------------------------------------------
! call stat_gs_sum(2)
! Putting that 2 in there is sooooooo fucking dodgy!
!-----------------------------------------------------------------------
            call stat_gs_sum(3)
            ltim = dnekclock() - ltim
...
!! @remark This routine uses global scratch space \a CTMP0
      subroutine stat_compute_1DMAX(lvar,npos,alpha,beta)
      implicit none
         ...
         do el = 1, nelv
            ...
            if(el2.gt.0) then
               do kl=1, stat_nm3
                  do jl=1, stat_nm2
                     do il=1, stat_nm1
				if(lvar(il,jl,kl,el).gt.rtmp(jl,kl,el2)) then
                        	rtmp(jl,kl,el2) = lvar(il,jl,kl,el)
				endif
                     enddo
                  enddo
               enddo
...
! --------------HARRY---------------------------------------------------
!=======================================================================
!> @brief Global statistics MAX
!! @ingroup stat
      subroutine stat_gs_max(maxed_variable)
      implicit none
      ...
      integer il, jl
      integer itmp1, itmp2
	integer maxed_variable
!-----------------------------------------------------------------------
      ! if no space averaging return
      if (stat_rdim.eq.0) return
      
      ! stamp logs
      call mntr_log(stat_id,lp_vrb,'Global statistics MAX.')
	...
	enddo
      ...
      ! MAX variables
      call fgslib_gs_op(gs_handle,stat_ruavg(1,1,maxed_variable),1,4,0)
...
!! @ingroup stat
      subroutine stat_gs_sum(stat_lvar_int)
      implicit none
      ...
      integer*8 unodes(lx1*lz1*lelt) ! unique local nodes
      integer il, jl
      integer itmp1, itmp2, stat_lvar_int
!-----------------------------------------------------------------------
      ! if no space averaging return
      if (stat_rdim.eq.0) return
...
!=======================================================================
      ...
      ! <1>t
      itmp = LX1*LY1*LZ1*LELT*LDIM
	call rone(tmpvel(1,1,1,1,1),itmp)	
	...
	! MAX<u>t
      lnvar = lnvar + 1
      npos = lnvar
      call stat_compute_1DMAX(slvel(1,1,1,1,1),npos,alpha,beta)
