...
c-----------------------------------------------------------------------
!     define the filter
      subroutine blob(field_fil,x,y,z,field)
      implicit none
      include 'SIZE'
      include 'INPUT'           ! IF3D
      include 'TSTEP'           ! IF3D
...
!     argument list
      real x,y,z
      real field_fil, field(NX1,NY1,NZ1,NELT)
!     local variable
...
!-----------------------------------------------------------------------
      ...
      do iel=1,nelt 
         do iz=1,NZ1
            do iy=1,NY1
               do ix=1,NX1
                  xdiff = (x-xM1(ix,iy,iz,iel))**2
                  ydiff = (y-ym1(ix,iy,iz,iel))**2
                  if (IF3D) then
                     zdiff    = (z-zm1(ix,iy,iz,iel))**2
                  ...
                  norm = norm + fkernel*bm1(ix,iy,iz,iel)
     ...
     $                    *bm1(ix,iy,iz,iel)
         ...
         enddo
      enddo
	field_fil = field_fil/norm
      return
      end
!***********************************************************************

!***********************************************************************
      subroutine filter_dodgyHarry(field_out,field_in)
      implicit none
      include 'SIZE'
...
!     local variables
      integer ix, iy, iz, iel
      real field_in(NX1,NY1,NZ1,NELT),field_out(NX1,NY1,NZ1,NELT)
      real x(3)
	real field_fil
!-----------------------------------------------------------------------
      ...
      do iel=1,NELT            
         do iz=1,NZ1
			...
			!     kernel of the filter
                  call blob(field_fil,x(1),x(2),x(3),field_in)
			field_out(ix,iy,iz,iel) = field_fil
		   enddo
            enddo
         enddo
      enddo
...
!!     local variables
!      integer ntot_ext
!	! THIS IS SUPER DODGY REMOVE AT SOME POINT WHEN IT'S GHOSTBUSTING TIME!!
!	nelt_ext = nelt
!	! THIS IS SUPER DODGY REMOVE AT SOME POINT WHEN IT'S GHOSTBUSTING TIME!!
